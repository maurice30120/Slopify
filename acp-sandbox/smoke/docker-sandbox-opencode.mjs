...
if (process.env.SLOPIFY_SBX_SMOKE !== '1') {
  console.log('Docker Sandbox OpenCode smoke test skipped. Set SLOPIFY_SBX_SMOKE=1 to run it.');
  process.exit(0);
}
...
const marker = 'opencode-smoke-marker.txt';
const scriptDirectory = dirname(fileURLToPath(import.meta.url));
const cli = resolve(scriptDirectory, '../../slopify/dist/src/cli.js');
const model = process.env.SLOPIFY_SBX_SMOKE_MODEL ?? 'opencode-go/glm-5.2';
...
try {
  ...
  writeFileSync(join(repo, 'README.md'), '# Docker Sandbox OpenCode smoke fixture\n');
  ...
  writeFileSync(join(repo, '.acp', 'acp-agents.json'), JSON.stringify({
    agents: {
      'OpenCode Sandbox': { transport: 'sandbox', agent: 'opencode', model },
    },
    pipeline: { enabled: true },
  }, null, 2));
  writeFileSync(join(repo, '.acp', 'pipelines', 'smoke.yaml'), `
version: 3
id: docker-sandbox-opencode-smoke
title: Docker Sandbox OpenCode smoke
promotion: ${action === 'promote' ? 'auto-apply' : 'auto-reject'}
nodes:
  - id: implement
    agent: OpenCode Sandbox
    prompt: >-
      Create exactly one file named ${marker} containing exactly
      docker sandbox opencode smoke followed by one newline. Do not modify any
...
`);
  ...
  const result = run(process.execPath, [
    cli,
    'run',
    'docker-sandbox-opencode-smoke',
    'Run the Docker Sandbox OpenCode smoke path.',
    '--cwd', repo,
    '--json',
  ...
  if (action === 'promote') {
    if (readFileSync(join(repo, marker), 'utf8') !== 'docker sandbox opencode smoke\n') {
    ...
    || readFileSync(join(repo, 'README.md'), 'utf8') !== '# Docker Sandbox OpenCode smoke fixture\n'
  ) {
