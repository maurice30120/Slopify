# 06: Documenter et nettoyer les commentaires du package `acp-sandbox/`

**What to build:** Tous les symboles exportés (classes, interfaces, fonctions, types) de `acp-sandbox/src/` reçoivent un bloc JSDoc en français décrivant leur contrat public, et les commentaires existants du package subissent le triple traitement : suppression des redondants (« quoi »), conservation et clarification des architecturaux (« pourquoi »), promotion en JSDoc des inline décrivant un contrat public. Le package passe l'audit avec zéro symbole exporté non documenté.

**Blocked by:** 01

**Status:** ready-for-agent

- [ ] Chaque classe, interface, fonction et type exportés de `acp-sandbox/src/` a un bloc JSDoc décrivant son rôle
- [ ] Le JSDoc suit le pattern existant : description courte, `@param`, `@return`, en français
- [ ] Les descriptions emploient le vocabulaire exact de `CONTEXT.md` (Sandbox Run, Promotion, Rejection, Cancellation, …) quand elles manipulent ces concepts
...
- [ ] L'audit ciblé sur `acp-sandbox/` (ticket 01) passe sans erreur